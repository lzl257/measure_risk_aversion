...
def szp(testh_model, data_name, initial_test, h_test, level):
    ...
    data respectively: estimators / t-values / R-square /

    relative risk aversion and corresponding confidence bond.

    Reports are generated in **bld/out/analysis**
    ...
    for key, data in y.items():
        popt_test, pcov_test = curve_fit(
            testh_model, x, np.array(data), initial_test, maxfev=2000)
        h_hat = popt_test[2]
        h_var = pcov_test[2, 2]
        ...
        if np.absolute(t_h0) < t_stat:
            ...
            predictions = np.array(
                [testh_model(i, a_hat, m_hat, h_test) for i in x.T])

...
def pop_data():
    ...
    pop_filsb = pop_filoc[(pop_filoc['Subject'].str.contains('Population mid-year estimates Total')) & 
...
def premiums_data():
    ...
    prem_fil = prem_raw[prem_raw['LOCATION'].str.contains('USA')][['TIME', 'Value']]
    prem_fil['Value'] = prem_fil['Value'] * 1e6   
    final_prem = prem_fil.rename(
            columns={'TIME': 'Year', 'Value': 'Premiums'}
            ).reset_index(drop=True)
...
def _known_growth(clam_table):
    ...
    for i in range(len(gclaims_list) - 1):
        growth_claims.append((gclaims_list[i+1] - gclaims_list[i]) / gclaims_list[i])
...
def _merge_clam(clam_table, guessed_claims):
    ...
    Returns:
        final_claims (pd.DataFrame): final claims table after replacing the outlier
...
def wealth_data():
    ...
    final_weal = weal_fil[['TIME', 'Value']].rename(
            columns={'TIME': 'Year', 'Value': 'Wealth'}
            ).reset_index(drop=True)
...
def _cpi_data():
    ...
    cpi_fil = cpi_raw[cpi_raw['LOCATION'].str.contains('USA')]\
                    [['TIME', 'Value']].reset_index(drop=True)
...
def save_data(known_claim, guess_claim, nonadj_table, cpi_table):
    ...
    Arg:
        ...
        guess_claim (pd.DataFrame): guessed claims based on the known growth rate
        nonadj_table (pd.DataFrame): claims/premiums/wealth with inflation
        cpi_table (pd.DataFrame): CPI data of the U.S.
